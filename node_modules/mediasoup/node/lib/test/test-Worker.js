...
test('createWorker() succeeds', async () => {
    ...
    worker1.close();
    await new Promise(resolve => worker1.on('subprocessclose', resolve));
    ...
    worker2.close();
    await new Promise(resolve => worker2.on('subprocessclose', resolve));
...
test('worker.updateSettings() succeeds', async () => {
    ...
    worker.close();
    await new Promise(resolve => worker.on('subprocessclose', resolve));
}, 2000);
...
test('Worker emits "died" if worker process died unexpectedly', async () => {
    ...
    if (!worker3.subprocessClosed) {
        await new Promise(resolve => worker3.on('subprocessclose', resolve));
    }
    expect(onDied).toHaveBeenCalledTimes(1);
    expect(onObserverClose).toHaveBeenCalledTimes(1);
